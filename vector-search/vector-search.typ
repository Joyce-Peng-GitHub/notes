...
= 基于 NSG 的 ANN 算法 #cite(label("10.14778/3303753.3303754"))
...
= 基于图的 ANNS 的综述与实验对比 #cite(label("10.14778/3476249.3476255"))
...
+ 作者关于基于图的 ANNS 算法讨论了推荐选项、指导原则、改进、趋势和挑战。
...
基于图的 ANNS 算法的虽然种类很多，但都遵循一个统一的处理管道。一个算法可以分为两个粗粒度组件，并进一步划分为七个个细粒度组件。

=== 构造

==== 初始化

#figure(caption: [不同构造策略的初始化方法])[
  #table(
    align: center,
    columns: 2,
    strong[策略], strong[初始化],
    [分治法], [数据集划分],
    [优化法], [邻居初始化],
    [增量法], [获取种子],
  )
]<tab:construction-initialization>

===== 数据划分

SPTAG 之前采用的是一种随机划分模式，即从数据集中随机抽取一组点，在上面生成主方向（？），然后执行随机划分来使每个子集的直径足够小。为了实现更优的划分，SPTAG 现在转向了 TP-树。

HCNNG 通过迭代执行层次化聚类来划分数据集。具体来说，他每次从要划分的集合中随机选取两个点，然后根据其他点到这两个点之间的距离来划分。

===== 邻居初始化

KGraph 和 Vamana 都是通过随机选择邻居来实现这一过程的。这一方法效率高，但得到的初始图的质量太低了。解决方案是通过基于哈希或树的 ANNS 方法来初始化邻居。

EFANNA 采用的是后者：在数据集上建立多棵 KD-树，然后对每个点做查询。这个方法高度依赖额外的索引，导致索引构建的代价增加。

因此，NSG、DPG 和 NSSG 采用了 NN-下降方法：首先为每个点随机选择邻居，然后通过邻居传播来更新每个点的邻居列表，最后通过少量迭代获得一张高质量的初始图。特别地，FANNG 和 IEH 通过线性扫描来初始化邻居。

===== 获取种子

另外两种策略在获得候选邻居时也包括这一过程，而且对于所有基于图的算法来说这一过程在搜索时都是必要的。

对于索引构建来说，NSW 和 NGT 都随机地获得种子，而 HNSW 由于其独特的层次化结构，其种子固定在顶层。

==== 获取候选邻居

用分治法构造的图一般在从通过数据集划分获得的小子集中产生候选邻居。STPAG 和 HCNNG 直接取划分的子集抠掉当前点作为候选邻居。

优化法和增量法不涉及数据集划分的过程，所以 IEH 和 FANNG 采用了朴素的方法，导致其索引构建效率低。为解决这一问题，NGT、NSW、HNSW、NSG 和 Vamana 都通过 ANNS 来获取候选邻居。优化法和增量法都是在现有图上对当前考虑的点执行 ANNS。这一方法只需访问一个数据集的小子集；然而，根据分析，通过 ANNS 获取候选邻居是杀鸡用牛刀，因为构造时要查的点属于数据集，但 ANNS 查询的点通常是不在数据集中的。作为对比，KGraph、EFANNA 和 NSSG 使用目标点在初始图上的邻居和邻居的邻居作为其候选邻居，这提高了索引构造效率。DPG 则直接使用当前图上目标点的邻居，但为了获得足够多的候选邻居，他通常要求初始图拥有更大的出度。

==== 选择邻居

当前的基于图的 ANNS 算法关于这一组件主要考虑两个因素：
- 距离：选取的邻居应离目标点尽可能近。
- 空间分布：邻居在目标点的各方向上应分布得尽可能均匀。

NSW、SPTAG、NGT、KGraph、EFANNA 和 IEH 都只考虑了距离因素，他们以构建一份高质量得图索引为目标。

原文在以下部分似乎犯了不少错误，我姑且保留我修正后的理解。

HNSW、FANNG、SPTAG 和 NSG（尽管 #cite(label("10.14778/3303753.3303754")) 区分了 HNSW 和 NSG 的邻居选择方式，但作者说他们在 @wang2021comprehensivesurveyexperimentalcomparison 中证明了二者的等价性）都通过计算邻居间的距离来考虑空间分布的因素。具体来说，考虑点 $p$，按照离 $p$ 的距离#strong[由近到远]考虑其候选点 $x$，$(p, x)$ 会被加入图中当且仅当
$
  (forall y in V(G) "s.t." (p, y) in E(G)) space (delta(x, y) > delta(x, p)).
$
#h(-indent) 这实质上是在使用 #link(label("def:monotonic-relative-neighborhood-graph"))[MRNG 的选边规则]。

为了更灵活地选取邻居，Vamana 添加了参数 $alpha >= 1$ 来控制邻居分布，即条件变为
$
  (forall y in V(G) "s.t." (p, y) in E(G)) space (alpha dot delta(x, y) > delta(x, p)).
$

DPG 选取候选邻居的子集来最小化所有邻居对的夹角之和。

NSSG 则设置一个夹角阈值 $theta$，$(p, x)$ 会被加入图中当且仅当
$
  (forall y in V(G) "s.t." (p, y) in E(G)) space (angle x p y >= theta).
$

NGT 使用路径调整来间接获得邻居的均匀分布。这种方法通过判断目标点在初始图上与其邻居之间是否存在替代路径来更新邻居。

HCNNG 通过在目标点与其候选点的集合上构建一棵 MST 来选取邻居。

==== 预处理种子

不同的算法对于预处理种子和连通性这两个组件可能存在不同的执行顺序。

===== 静态方式

典型代表有 HNSW、NSG、
- HNSW 固定顶层顶点为种子。
- NSG 和 Vamana 使用数据集的近似质心（似乎有细节问题：根据 @algo:nsg-construction，#cite(label("10.14778/3303753.3303754")) 实际上是对质心求最近邻作为近似的#link(label("def:medoid"))[中心点]）.
- NSSG 的种子是随机选取的顶点。

===== 动态方式

一种常见的操作是附加其他索引（即对于每个查询，通过额外的索引来获得靠近查询点的种子）。

- SPTAG、EFANNA、HCNNG 和 NGT 构建额外的树，例如 KD-树、平衡 K-均值树 和 VP-树。
- IEH 通过哈希来为获取种子做准备。
- 有一篇论文通过 OPQ（不知道是甚么）来压缩原始向量，以通过快速计算压缩过的向量来获取种子。
- KGraph、FANNG、NSW 和 DPG 都随机地获取种子，也就不需要实现种子预处理。

==== 连通性

- 分治法通常通过多次执行数据集划分和子图构建来保证连通性，例如 SPTAG。
- 优化法通常附加深度优先遍历来实现连通性，例如 NSG。
- 增量法天然能保证连通性，例如 NSW。
...
因为种子对搜索有显著影响，这一组件比增量策略的初始化更受人关注。一些早期算法随机获取种子，而 SOTA 算法们一般使用预处理种子的方式。如果在预处理阶段能够产生固定的种子，他（们？）在这一组件中就可以直接加载。

==== 路由

几乎所有基于图的 ANNS 都基于贪心路由策略，包括最佳优先搜索（维护最靠近目标的若干个点，每次选取其中最近的进行扩展）及其变体。NSW、HNSW、KGraph、IEH、EFANNA、DPG、NSG、NSSG 和 Vamana 都使用原本的最佳优先搜索来路由。尽管这一方法方便部署，他有两个短板：
+ 容易陷入局部最优。
+ 路由效率低下。

第一个短板摧毁了搜索结果的准确度。为缓解这一问题：
- ANNG 为最佳优先搜索增加了回溯，这可以轻微提高搜索准确度，但会显著增加搜索时间。
- NGT 添加参数 $epsilon$：他取消了对候选池的大小上限，取池中离目标点的最远距离为搜索半径 $r$，当池中最近点的邻居到目标的距离小于 $(1 + epsilon) dot r$ 时就将该邻居加入候选池。增大 $epsilon$ 可以缓解该问题，但也会显著增加搜索时间。
- SPTAG 迭代执行最佳优先搜索：当一次迭代陷入局部最优时，他会从 KD-树上选取新的种子来重新搜索。

HCNNG 提出使用有引导的搜索来缓解第二个问题，而非像最佳优先搜索那样访问最佳候选点的所有邻居，所以有引导的搜索基于查询点的位置避免了一些多余的访问。

== 实验评估

作者希望通过评估来回答以下问题：
+ 算法在不同场景下性能如何？
+ 一个算法能同时有最佳的索引构建和搜索性能吗？
+ 对于一个有最优总体性能的算法，其每个细粒度组件的性能也都是最优的吗？
+ 基于机器学习的优化（？）如何影响基于图的算法的性能？
+ 我们如何能基于实验观察来设计一个更好的基于图的算法并验证其性能？
...
#figure(caption: [不同场景下推荐使用的算法])[
  #table(
    align: left,
    columns: 2,
    strong[场景], strong[算法],
    [频繁更新大量数据], [NSG, NSSG],
    [快速构建 KNNG], [KGraph, EFANNA, DPG],
    [数据存储在外存中], [DPG, HCNNG],
    [在困难的 #footnote[局部内在维度（local intrinsic dimensionality, LID）较大的。] 数据集上搜索], [HNSW, NSG, HCNNG],
    [在简单的数据集上搜索], [DPG, NSG, HCNNG, NSSG],
    [GPU 加速], [NGT],
    [存储资源有限], [NSG, NSSG],
  )
]<tab:algo-recommendation-in-diff-scenarios>

=== 指导原则

从直觉上来说，一个实用的基于图的 ANNS 算法应当有
+ 高的构造效率。

  不应该花费过多时间来提高图质量，因为最高的图质量并非最优搜索性能所必需的。

+ 高的的路由效率。

  + 应当适当地控制出度，多样化邻居的分布，以及减小获取入口的代价。
  + 应当通过优化路由策略来避免多余的距离计算。

+ 高的搜索准确度。

  为提高搜索对于陷入局部最优的免疫力，应当在合理设计构造时邻居的分布，保证连通性，并优化路由策略。

+ 低的存储开销。

  通过改进邻居选择方式和路由策略来减小出度和候选集尺寸。

=== 改进

作者通过超级组合各种组件，设计了一种算法 @wang2021comprehensivesurveyexperimentalcomparison，能同时满足上面的四条指导原则。

=== 趋势

作者注意到，几乎所有 SOTA 算法都基于 RNG，例如 HNSW 和 NSG，因此许多方法在基于 KNNG 或 DG 的算法的基础上添加了一张近似 RNG。基于 RNG 的类型对于基于图的 ANNS 来说仍然是一个有前景的研究方向。基于 MST 的算法最近呗用于基于图的 ANNS，而且在作者的评估中也达到了优秀的结果，特别是在困难的数据集上。

在这篇论文讨论的核心算法的基础上，研究者们正在通过硬件来优化和改进算法性能。其他文献则通过增加量化编码方案或分布式方案来应对数据增加。为了满足混合的查询要求，最新的研究为搜索过程增加了结构化的属性约束。

=== 挑战

当前，几乎所有基于图的算法都是面向原始数据的，这也是现在这些算法存储使用高的主要原因。如何将数据编码或其他方法与基于图的 ANNS 算法有机结合起来，这是一个指的探索的问题。相比于树、哈希和量化，基于图的算法有着最高的索引构建时间，这也为实时更新图索引增加了挑战。还有，如何结合 GPU 加速或其他方法也是指的深入研究的。

一个主要的挑战是如何通过学习（？）以根据数据集的特点随机应变地选择最优的算法。
